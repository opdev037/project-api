...
COPY composer.json ./
COPY composer.lock* ./
# --ignore-platform-reqs：composer 這個 stage 本身沒有 ext-rdkafka，
# 但實際擴展會在 Stage 2 安裝，這裡只負責解析/下載套件
...
# 安裝 Redis + Kafka 擴展
# librdkafka 為 rdkafka 擴展的執行期依賴，需保留（不能跟著 build-deps 一起刪除）
